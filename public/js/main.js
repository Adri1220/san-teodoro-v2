document.addEventListener("DOMContentLoaded", async () => {
   ...
   // ==========================================================================
   // 2. CONEXIÓN A TU BASE DE DATOS (obras.json)
   // ==========================================================================
   let obras = {};
   try {
      ...
      return; // Si no hay datos, detenemos todo para no romper la web
   }

   // ==========================================================================
   // 3. GENERADOR DE CATÁLOGO Y FILTROS
   // ==========================================================================
   ...
   if (catalogGrid && Object.keys(obras).length > 0) {
      ...
      const renderizarCatalogo = () => {
         ...
         catalogGrid.innerHTML =
            tarjetasHTML ||
            `<p style="grid-column: 1/-1; text-align:center; color: var(--color-text-muted); font-family: var(--font-sans); padding: 40px;">No se encontraron resultados.</p>`;
   ...
   // ==========================================================================
   // 4. INYECTOR DE FICHAS ARCA (Obras y Nichos)
   // ==========================================================================
      ...
      if (id && obras[id]) {
         ...
         if (isObra || isNicho) {
            ...
            if (isObra && datos.modelo3d) {
               document.getElementById("visor3d-container").innerHTML = `
                  <div style="position: relative; width: 100%; aspect-ratio: 4/3; border: 1px solid var(--color-border-marble); background-color: #f6f3f2; overflow: hidden;">
                     <div id="loader-3d" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; display: flex; flex-direction: column; justify-content: center; align-items: center; z-index: 1;">
                        <div class="spinner"></div>
                        <span style="font-family: var(--font-sans); font-size: 12px; font-weight: 600; color: var(--color-text-muted); letter-spacing: 0.1em; text-transform: uppercase; margin-top: 16px;">Cargando modelo...</span>
                     </div>
                     <iframe title="Modelo 3D de ${datos.titulo}" frameborder="0" allow="autoplay; fullscreen; xr-spatial-tracking; gyroscope; accelerometer" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; z-index: 2; opacity: 0; transition: opacity 0.5s ease;" src="${datos.modelo3d}" onload="document.getElementById('loader-3d').style.display = 'none'; this.style.opacity = '1';"></iframe>
                  </div>
         ...
         pageContainer.innerHTML = `
            <div style="text-align: center; padding: 100px 20px; display: flex; flex-direction: column; align-items: center; gap: 24px;">
               ...
               <p style="font-family: var(--font-sans); font-size: 16px; color: var(--color-text-muted); max-width: 500px; line-height: 1.6;">Lo sentimos, la obra que intentas visualizar no existe en la base de datos.</p>
               <a href="/colecciones" class="btn btn--primary" style="margin-top: 16px;">Volver a Colecciones</a>
            </div>
         `;
      }
   }
